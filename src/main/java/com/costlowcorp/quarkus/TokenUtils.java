 ...
 */
package com.costlowcorp.quarkus;
...
public class TokenUtils {
     ...
     * @throws Exception on parse failure
     */
    public static String generateTokenString(Map<String, Object> map, Map<String, Long> timeClaims)
            throws Exception {
        // Use the test private key associated with the test public key for a valid signature
        PrivateKey pk = readPrivateKey("privateKey.pem");
        return generateTokenString(pk, "privateKey.pem", map, timeClaims);
    }
    ...
    public static String generateTokenString(PrivateKey privateKey, String kid,
        ...
        var expiration = 5 * 24 * 60 * 60;
        long exp = timeClaims != null && timeClaims.containsKey(Claims.exp.name())
            ? timeClaims.get(Claims.exp.name()) : currentTimeInSecs + expiration;
    ...
    public static String generateTokenString(PrivateKey privateKey, String kid,
        Map<String, Object> jsonResName, Map<String, Long> timeClaims) throws Exception {

        JwtClaimsBuilder claims = Jwt.claims(jsonResName);
        long currentTimeInSecs = currentTimeInSecs();
        var expiration = 5 * 24 * 60;
        long exp = timeClaims != null && timeClaims.containsKey(Claims.exp.name())
            ? timeClaims.get(Claims.exp.name()) : currentTimeInSecs + expiration;
